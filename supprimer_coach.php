...
try {
    ...
    // Récupérer tous les coachs de la base de données
    $stmt = $cx->prepare("SELECT * FROM coach");
    $stmt->execute();
    $coachs = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Supprimer un coach si le formulaire est soumis
                ...
                <tbody>
                    <?php foreach ($coachs as $coach): ?>
                        <tr>
                            ...
                            <td>
                                <form action="" method="post" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce coach ?');">
                                    <input type="hidden" name="email" value="<?php echo htmlspecialchars($coach['mail']); ?>">
                                    <button class="btn-20">
